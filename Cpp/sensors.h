...
class loadCell{
  public:
    ...
    int DOUT;
    int CLK;
    float calibration_factor = -7050; // is this optimal? does this change at max thrust vs other values of thrust?
    float weight;
  // Constructor
        ...
        if( currentstate == LOW ) // if the state changed 
          {
            ...
            rpm = ((60000000/duration)/2); // rpm = (freq)*1000000*60; Check This please, i didnt test or change anything and don't know how it works
            prevmicros = micros(); 
          }
      }
